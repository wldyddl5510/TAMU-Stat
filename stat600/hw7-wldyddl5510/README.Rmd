...
$$
L_{\tau}(L, S, \nu) = \{||L||_{*} + \gamma ||S||_{1} + tr(\nu^{T}(L + S - M)) + \frac{1}{2 \tau} ||L + S - M||^{2}_{F} \}
$$
...
**Update of L:**
$$
L^{t+1} = \arg\min_{L}\{\mathcal{L}_{\tau}(L, S_t, \frac{\eta_t}{\tau}) \}, \: (\text{Where }\nu_t = \frac{\eta_t}{\tau})
$$
...
(\text{Where }UDV^T \text{ is SVD of }M - S_t - \eta_t) \nonumber 
\end{align}

**Update of S:**

$$
S^{t+1} = \arg\min_{S} \{\mathcal{L}_{\tau}(L_{t+1}, S, \frac{\eta_t}{\tau}) \}, \: (\text{Where }\nu_t = \frac{\eta_t}{\tau})
$$
which is equivalent to (derive the explicit update and fill in below)

\begin{align}
S^{t+1} = \arg\min_{S} \{ \gamma ||S||_{1} + tr((\frac{\eta_t}{\tau})^{T} (L_{t+1} + S - M) + \frac{1}{2 \tau} tr((L_{t+1} + S - M)^T (L_{t+1} + S - M)) \} \\
